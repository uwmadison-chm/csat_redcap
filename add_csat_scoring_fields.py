...
Code to generate a REDCap instrument for an automatically-scored version of
the Constitutional Self-Assessment Tool.

This code relies on two files:

inst_base.csv     : a REDCap instrument that is the base questions for the CSAT,
                    should have 50 entries
csat_scoring.txt  : A computer-readable text file that has scoring information
                    for the CSAT questions

There's a ton of special-casing in this file. It's appalling code. But really,
it just had to work once and the instrument is configured.
...
DOC_Q = {
    "Variable / Field Name": "csat_build_info",
    "Form Name": "csat",
    "Field Type": "descriptive",
    "Field Label": "Do not edit this instrument! It was generated by a script found here: https://github.com/uwmadison-chm/csat_redcap",
    "Field Annotation": "@HIDDEN-SURVEY"
}

# Menstruation questions are a separate thing
MENS_Q_PRE = [
...
# So we can hide the menstruation questions and change the denominator for
...
DENOM['Choices, Calculations, OR Slider Labels'] = f'if({MENS_BRANCH_LOGIC}, 50, 48)'
...
def parse_score_file(score_file):
    """
    Turn the text file into a list of scoring information. There should be
    one scoring definition per question -- so 50 for each for this instrument.
    """
    file_data = ""
...
def make_check_calc(row, category, values):
    """
    Take a checkbox field and output a calculated field for every category
    The calculation will be the mean of the checked options for a category;
    in this case that scores each checked option at 0.5
    """
...
def make_radio_calc(row, category, values):
    """
    Take a radio field and output a calculated field for every category
    """
    orig_field = row['Variable / Field Name']
    conditions = [
        f"[{orig_field}] = \"{val}\""
...
def make_upgraded_instrument(score_inst, instrument_df):
    """
    Turn the scoring instructions and base instrument into a big instrument
    """
    output = []
    output.append(DOC_Q)
